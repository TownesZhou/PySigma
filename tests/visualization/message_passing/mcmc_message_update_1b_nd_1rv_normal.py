...
if __name__ == "__main__":
    ...
    b_shape, p_shape, s_shape, e_shape = Size([1]), Size([]), Size([num_ptcl]), Size([2])
    ...
    for i in range(num_iter):
        ...
        # Massage log density tensor shape: prepend batch dimension
        post_log_prob, eval_log_prob = post_log_prob.unsqueeze(dim=0), eval_log_prob.unsqueeze(dim=0)
    ...
    plot_grid_flattened = plot_grid.view(-1, plot_grid.shape[-1])
    # Distribution curve
