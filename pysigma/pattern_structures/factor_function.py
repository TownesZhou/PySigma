...
class FactorFunction:
    """
        ...
        a list of discriminative functions generating particles for each of the random variables::
        ...
        This discriminative form is useful when it is easy to establish deterministic functional relationships between
        ...
        densities are retrieved, whereas for others, the particle values are retrieved. However, it is required that
        ...
        will index the batch dimension. Therefore, the factor function callback should not modify this dimension. For
        the generative factor function, the callback shall return a 1-dimensional vector with non-negative values
        representing the joint probability densities. The vector shall have a same size as the input tensors' batch
        size. For the discriminative factor function, the callback shall return a 2-dimensional tensor, with the
        first dimension's size equal to the input tensors' batch size and the second dimension's size equal to its
        corresponding random variable's event size.
    ...
    def __init__(self,
        ...
        # Check that if prob_func is declared, vector_func is not fully specified.
            ...
            assert vector_func is None or any(vf is None for vf in vector_func), \
                "When `prob_func` is specified, `vector_func` cannot be fully specified."
    ...
    def directional_vars(self) -> Tuple[Variable]:
        ...
        return tuple(rv for i, rv in enumerate(self.ran_vars) if self._vector_func[i] is not None)

    def __call__(self, inputs: IterableType[torch.Tensor], func_type: str = 'generative',
                 node_device: torch.device = 'cpu') -> Union[torch.Tensor, Tuple[torch.Tensor]]:
        """
            ...
            node_device : torch.device
                The calling node's device. Output tensor from factor function callback will be converted to this device.

            Returns
            -------
            torch.Tensor or tuple of torch.Tensor
                If `func_type` is ``"generative"``, then returns a single tensor representing the batched joint
                probability densities. Otherwise, returns a tuple of tensors, each representing the particles output
                of a declared vector function corresponding the random variables in `self.directional_vars` in order.
        """
        # Validate arguments
        ...
        # Declarative function
            ...
            # Iterate through vector function and its corresponding random variable
            for i, (ran_var, vector_func) in enumerate(zip(self.ran_vars, self._vector_func)):
                if vector_func is not None:
                    # Remove the input tensor that corresponds to this random variable
                    discriminative_inputs = inputs[:i] + inputs[i+1:]
                    ...
                    try:
                        ...
                        raise FactorFunctionRuntimeError(
                            ...
                            "the random variable {}.".format(vector_func, ran_var)
                        ) from exc
                    # Validate result
                    if not isinstance(result, torch.Tensor):
                        raise FactorFunctionValueError(
                            ...
                            "variable {}.".format(type(result), vector_func, ran_var)
                        )
                    ...
                    if not result.ndim == 2:
                        raise FactorFunctionValueError(
                            ...
                            .format(result.ndim, vector_func, ran_var)
                        )
                    if not result.shape == torch.Size([b_size, ran_var.size]):
                        raise FactorFunctionValueError(
                            "The returning tensor from a discriminative callback must represent batched particles. "
                            "Expect a tensor with shape {}. Instead, found a tensor with shape {} in the "
                            "discriminative factor function {} corresponding to the random variable {}."
                            .format(torch.Size([b_size, ran_var.size]), result.shape, vector_func, ran_var)
                        )
